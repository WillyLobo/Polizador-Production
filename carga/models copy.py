...
class Poliza(models.Model):
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["legacy_poliza_fecha", "legacy_poliza_numero", "legacy_poliza_aseguradora","legacy_poliza_receptor"], name="legacy_poliza-constraint")]
        verbose_name = "Póliza"
        verbose_name_plural = "Pólizas"

    #poliza_fecha            = models.DateField("Fecha")
    p#oliza_expediente       = models.CharField("Número de Expediente", max_length=17)
    poliza_receptor_legacy         = models.ForeignKey("Receptor", on_delete=models.CASCADE)
    poliza_area_legacy             = models.ForeignKey("Area", on_delete=models.CASCADE)
    ...
    poliza_obra_nombre_legacy      = models.TextField("Obra")
    poliza_obra_convenio_legacy    = models.CharField("Convenio", max_length=50, blank=True, null=True)
    poliza_obra_expediente_legacy  = models.CharField("Número de Expediente de la Obra", max_length=17, blank=True, null=True)
    poliza_monto_pesos_legacy      = models.DecimalField("Monto Sustituido Pesos", max_digits=12, decimal_places=2, blank=True, null=True, validators=[MinValueValidator(0)])
    poliza_monto_uvi_legacy        = models.DecimalField("Monto Sustituido UVI", max_digits=12, decimal_places=2, blank=True, null=True, validators=[MinValueValidator(0)])
